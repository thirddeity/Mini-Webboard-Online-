...
const sessions = require('express-session'); // copy จากในเว็บมา เปลี้ยนจาก app เป็น router
router.use(sessions({
  secret:"abc", // ใส่อะไรก็ได้
  saveUninitialized:true,
  cookie:{ maxAge: 100 * 60 * 24 * 30}, // login อยู่ได้30วัน
  resave:false
}));
...
router.use((req,res,next) => {
  res.locals.session = req.session ; //เอาตัวแปร session เก็บเข้าไปในตัวแปร locals
  next();
})
...
// เช็ค user , pass จาก database ว่ากรอกถูกต้องไหม
  ...
  if (req.body['user'] != undefined) {
    ...
    conn.query(sql,params,(err,result) => {
      ...
      if (result[0].id != undefined){
        req.session.user_id = result[0].id; // 1.เขียนหลังทำ require sessions จากข้างบนแล้ว
        req.session.user_name = result[0].name; // 2. เขียนเพื่อเก็บ user.id กับ name ไว้ และไปที่หน้า board ทันที
...
/* ใส่ if else เช็คว่าต้องกรอกข้อมูล name เพื่อบันทึกลง Database 
  จากนั้นแสดงข้อความไปที่หน้า register_success
*/
...
/* สร้าง register_success มารองรับข้อมูลจากหน้า register ข้างบน */
...
router.get('/comment/:topic_id', (req,res) => {
  ...
  conn.query(sql , params,(err,result) => {
    if(err) throw err;
    let topic = result[0] ; //แสดงเป็น array
    
    sql = '' +
    ' SELECT * FROM tb_comment ' + 
    ' LEFT JOIN tb_member ON tb_member.id = tb_comment.member_id'
    ' WHERE tb_comment.topic_id = ? ORDER BY tb_comment.id DESC' ; //DESC ตัวเก่าข้างล่าง ตัวใหม่ข้างบน
    conn.query(sql,params,(err,result) => {
      if (err) throw err;
